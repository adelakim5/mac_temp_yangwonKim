...
describe('<App />', () => {
  ...
  it('delete todo-items', () => {
    ...
    fireEvent.change(input, { target: { value: 'study react 3' } });
    fireEvent.click(button); // study react 3을 추가한 상태

    const todoList = screen.getByTestId('todoList');
    const todoItem = screen.getByText('study react 3');
    const deleteButtons = screen.getAllByText('삭제'); // todoitem이 입력되었으므로 삭제 버튼이 나올 것

    fireEvent.click(deleteButtons[0]);
    expect(todoItem).not.toBeInTheDocument();
    expect(todoList.childElementCount).toBe(2);
    // study react 1, study react-test 두개가 남아있으니까
  });
});
